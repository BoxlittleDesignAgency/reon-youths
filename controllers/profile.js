...
const getProfileById = async ({ params: { userId } }, res) => {
  console.log('userId: ', userId);

  console.log('UserId Handle: ', userId);

  // await Profile.findOne({user: userId}, (err, profile) => {
  //   console.log(profile)
  // })
  ...
  Profile.findById(userId)
    ...
    .catch((err) =>
      res.status(404).json({
        noprofilefound: 'No profile found with that ID.'
      })
    );
  ...
  //   })
  //   .catch((err) =>
  //     res.status(404).json({ profile: 'Error getting the profile' })
  //   );
};
